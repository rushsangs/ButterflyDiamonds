...
<script type="text/javascript" src="style/js/fancybox/jquery.fancybox.pack.js"></script>
...
<body id="page">
	...
	<?php 
	$stmt2=$mysqli->prepare("SELECT approved FROM members where username=? LIMIT 1");
					$stmt2->bind_param('s',$_SESSION['username']);
                    $stmt2->execute();   // Execute the prepared query.
            		$stmt2->store_result();
                    $stmt2->bind_result($approved);
					$stmt2->fetch();
	if (login_check($mysqli) == true && $approved=="yes") { ?>
		<p id="new"></p>
		<p id="test"></p>
		<!-- Begin Toggle -->
	...
	<script type="text/x-kendo-template" id="template2">
          ...
          <div class="thumb">
          	<a href= #= PHOTO # class="showdets hover fancybox-media" rel="media-gallery"  title= #= PID #><span class="overlay zoom"></span><img src= #= PHOTO #  style="display: inline" onerror="this.src='style/images/imgnotavailable.jpg'" width='280px'></a> 
          <div class="product">
          		   <table >
          ...
          <div class="details">
           ...
           <input type='checkbox' class='checkbox1' value='select1' style='float: left; display: inline' data-bind='checked: SELECT' #= SELECT ? checked='checked' : '' # />
          	<h4 class="entry-title" style="display: inline"> #= PID #</h4>
          </div>
						</div>	
            </script>
                ...
                <div id="tabstrip">
                    <div>
                        <div class='employee-details'>
							...
							<td>
								<img src= #= PHOTO #  style="display: inline" onerror="this.src='style/images/imgnotavailable.jpg'" class="img1" width='300px'> 
							</td> 
							</tr>
							</table> 
       ...
       <script>
		...
		source=new kendo.data.DataSource({
           ...
           function dotilestuff()
           {
           	...
           	//comment out the next line if you want items with no images to be displayed as well
           	prepareDataForTile();
           ...
           function initTile()
           {
            ...
            $(".checkbox1").change(function(e)
            {
    				...
    				var listitemrow=list.select();
 		   			var uid = listitemrow.data("uid");
					var listitem = list.dataSource.getByUid(uid);
    				listitem.SELECT=!listitem.SELECT;
           ...
           function sendmail(wholedata,wholedata2,form)
           {
           		var selecteddata=[];
           		...
           		for(var i=0;i<wholedata2.length;++i)
           		{
           			
           			if(wholedata2[i].SELECT==true)
           			{
           				selecteddata.push(wholedata2[i]);
           			}
           		}
...
for(var i=0; inputElements[i]; ++i){
      if(inputElements[i].className==="messageCheckbox" && 
        ...
        <?php }elseif(login_check($mysqli) == true) {
        	?>
        	<p>Your account has not yet been approved. Please contact P. Hirani diamonds to get your account approved.Your username is <?php echo $_SESSION['username'];?>	</p>	
  <?php }else { ?>
            <p>
